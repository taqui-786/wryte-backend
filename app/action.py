...
async def get_user_documents(user_id: uuid.UUID, session: AsyncSession):
    try:
        result = await session.execute(select(Doc).where(Doc.user_id == user_id))
...
async def get_document_by_id(document_id: str, session: AsyncSession):
    try:
        result = await session.execute(
            select(Doc)
            .options(selectinload(Doc.threads))
...
async def add_new_thread(
    thread_id: str, doc_id: uuid.UUID, title: str, session: AsyncSession
):
    try:
        thread = Thread(
            id=thread_id,
